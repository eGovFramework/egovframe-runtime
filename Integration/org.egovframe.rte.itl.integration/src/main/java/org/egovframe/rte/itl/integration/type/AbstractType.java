...
public abstract class AbstractType implements Type {
     ...
     * @param assignableClasses assign할 수 있는 Java 객체의 Class들
    ...
    protected AbstractType(final String id, final String name,
        ...
        if (!StringUtils.hasText(id)) {
            LOGGER.debug("### AbstractType id does not have a text (id = \"{}\")", id);
            throw new IllegalArgumentException();
        } else if (!StringUtils.hasText(name)) {
            LOGGER.debug("### AbstractType name does not have a text (name = \"{}\")", name);
            throw new IllegalArgumentException();
        } else if (assignableClasses == null || assignableClasses.isEmpty()) {
            LOGGER.debug("### AbstractType assignableClasses is null or empty (assignableClasses = {})", assignableClasses);
            throw new IllegalArgumentException();
        }
        this.id = id;
        this.name = name;
     ...
     * @throws IllegalArgumentException Argument <code>id</code> 값이 <code>null</code>이거나, 공백 문자로만
     *                                  이루어진 경우
     */
     ...
     * @throws IllegalArgumentException Argument <code>name</code> 값이 <code>null</code>이거나, 공백 문자로만
     *                                  이루어진 경우
     */
     ...
     * assignableClasses
     *
     * @param assignableClasses the assignableClasses to set
     * @throws IllegalArgumentException Argument <code>assignableClasses</code> 가 <code>null</code>
     *                                  이거나 빈 Array인 경우
     */
    ...
    public boolean isAssignableFrom(final Class<?> clazz) {
        if (clazz == null) {
            LOGGER.debug("### AbstractType Argument 'clazz' is null");
            throw new IllegalArgumentException();
        }
