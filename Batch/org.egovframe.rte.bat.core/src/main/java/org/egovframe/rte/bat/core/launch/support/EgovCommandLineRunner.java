 ...
 * 2023.08.31	유지보수			NullPointerException를 분리(start(), Contribution 반영)
 * </pre>
 * @since 2012.06.28
 */
public class EgovCommandLineRunner {
    ...
    public int start(String jobPath, String jobIdentifier, String[] parameters, Set<String> opts) {
        ...
        try (ConfigurableApplicationContext context = new ClassPathXmlApplicationContext(jobPath)) {
            ...
            Assert.isTrue(ObjectUtils.isEmpty(parameters) || !jobParameters.isEmpty(), "Invalid JobParameters " + Arrays.asList(parameters) + ". If parameters are provided they should be in the form name=value (no whitespace).");
        ...
        } catch (NullPointerException | JobParametersNotFoundException | JobInstanceAlreadyCompleteException |
                 NoSuchJobException | JobExecutionNotFailedException | JobExecutionNotStoppedException |
                 JobExecutionAlreadyRunningException | JobExecutionNotRunningException | JobParametersInvalidException |
                 JobRestartException e) {
            EgovCommandLineRunner.message = "Job Terminated in error: " + e.getMessage();
    ...
    private List<JobExecution> getJobExecutionsWithStatusGreaterThan(String jobIdentifier, BatchStatus minStatus) {
        ...
        while (!lastInstances.isEmpty()) {
            for (JobInstance jobInstance : lastInstances) {
                ...
                if (ObjectUtils.isEmpty(jobExecutions)) {
                    continue;
                }
                for (JobExecution jobExecution : jobExecutions) {
                    if (jobExecution.getStatus().isGreaterThan(minStatus)) {
    ...
    private JobParameters getNextJobParameters(Job job) throws JobParametersNotFoundException {
        ...
        if (lastInstances.isEmpty()) {
            ...
            if (ObjectUtils.isEmpty(jobParameters)) {
