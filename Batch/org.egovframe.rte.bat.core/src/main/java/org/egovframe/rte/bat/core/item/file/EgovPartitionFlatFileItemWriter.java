...
public class EgovPartitionFlatFileItemWriter<T> extends ExecutionContextUserSupport implements ResourceAwareItemWriterItemStream<T>, InitializingBean {
    ...
    /**
     * file 이 열리고 닫힘에 다라 Counting, 0 일경우 정상적인 Closing이 가능
     */
    private int fileCount = 0;

    /**
     * file이 최초로 open 될 때의 시간
     */
    private long fileOpenTime = 0;

    /**
     * file이 최종으로 close 될 때의 시간
     */
    ...
    /**
     * Sets the writable resource that will be used by this writer.
     * This resource typically represents the target file or stream
     * where data will be written.
     *
     * @param resource the {@code WritableResource} to set, which
     *                 represents the output destination
     */
    @Override
    ...
    /**
     * Writes the provided chunk of items to the output resource.
     * This method is typically invoked during the partitioned processing
     * of items, ensuring that the chunk is persisted correctly.
     *
    ...
    public synchronized void write(List<? extends T> items) throws Exception {
        ...
        LOGGER.debug("### EgovPartitionFlatFileItemWriter write() Writing to flat file with {} items", items.size());
    ...
    public synchronized void close() {
        ...
        if (state != null && fileCount == 0) {
            ...
            } catch (ItemStreamException | IOException e) {
    ...
    private class OutputState {
        ...
        private Writer getBufferedWriter(FileChannel fileChannel, String encoding) {
            ...
            try {
                if (transactional) {
                    TransactionAwareBufferedWriter tabw = new TransactionAwareBufferedWriter(fileChannel, new Runnable() {
                        public void run() {
                            closeStream();
                        }
                    });
