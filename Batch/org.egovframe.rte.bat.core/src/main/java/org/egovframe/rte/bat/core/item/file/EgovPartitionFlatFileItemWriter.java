...
public class EgovPartitionFlatFileItemWriter<T> extends ExecutionContextUserSupport implements ResourceAwareItemWriterItemStream<T>, InitializingBean {
	...
	public synchronized void close() {
		...
		if (state != null && fileCount == 0) {
			try {
				...
				ReflectionUtils.handleReflectionException(ie);
			} catch (IOException e) {
	...
	private class OutputState {
		...
		private Writer getBufferedWriter(FileChannel fileChannel, String encoding) {
			Writer writer = null;
			TransactionAwareBufferedWriter tabw = null;
			try {
				writer = Channels.newWriter(fileChannel, encoding);
				if (transactional) {
					tabw = new TransactionAwareBufferedWriter(fileChannel,new Runnable() {
								public void run() {closeStream();
								}
							});
