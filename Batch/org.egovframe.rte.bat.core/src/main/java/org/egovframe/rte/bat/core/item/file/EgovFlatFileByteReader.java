...
public class EgovFlatFileByteReader<T> extends AbstractItemCountingItemStreamItemReader<T> implements ResourceAwareItemReaderItemStream<T>, InitializingBean {
    ...
    protected void doOpen() throws Exception {
        ...
        if (!resource.exists()) {
            ...
            LOGGER.debug("### EgovFlatFileByteReader doOpen() Input resource does not exist {} ", resource.getDescription());
            return;
        }
        ...
        if (!resource.isReadable()) {
            ...
            // slf4J logger 로 변경 : 2014.04.30
            LOGGER.debug("### EgovFlatFileByteReader doOpen() Input resource is not readable {} ", resource.getDescription());
            return;
        }
