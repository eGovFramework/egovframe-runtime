 ...
 * @since 2013.04.01
 */
public class EgovSchedulerJCRunnerTest {
    ...
    public void testContinueStart() {
        assertTimeoutPreemptively(Duration.ofSeconds(15), () -> {
            JCThreadRunner runner = new JCThreadRunner();
            runner.start();
...
class JCThreadRunner extends Thread {
    private EgovSchedulerJCRunner egovSchedulerJCRunner;

    @Override
    public void run() {
        egovSchedulerJCRunner = new EgovSchedulerJCRunner(
                new Class<?>[]{LaunchScheduleJCConfig.class},
                -2000
        );
        egovSchedulerJCRunner.start();
