...
import org.egovframe.rte.fdl.filehandling.config.VfsFileSystemManagerFactory;
...
public class FilehandlingServiceTest {
    ...
    public void testCp() throws IOException {
        ...
        EgovFileUtil.cp(filename, tmppath + "/test2.txt");

        assertEquals(
                EgovFileUtil.readFile(new File(filename), "UTF-8"),
                EgovFileUtil.readFile(new File(tmppath + "/test2.txt"), "UTF-8")
        );
    }
    ...
    public void testMv() throws IOException {
        EgovFileUtil.writeFile(tmppath + "/" + filename, text, "UTF-8");

        EgovFileUtil.mv(tmppath + "/" + filename, tmppath + "/movedfile.txt");

        assertEquals(text, EgovFileUtil.readFile(new File(tmppath + "/movedfile.txt"), "UTF-8"));
    }
    ...
    public void testCd() throws IOException {
        ...
        String targetUri = foldername.getURI();
        EgovFileUtil.cd("");

        String uri = EgovFileUtil.pwd().getURI();
        assertNotEquals(targetUri, uri);

        EgovFileUtil.cd(targetUri);
    ...
    public void testUserAuthentication() throws IOException {
        ...
        FileObject baseDir = manager.resolveFile(System.getProperty("user.dir"));
        FileObject file = manager.resolveFile(baseDir, "testfolder/file1.txt");
        FileObject fo = manager.resolveFile(file.getURL().toExternalForm(), opts);
        fo.createFile();
    }
    ...
    public void testCaching1() throws IOException {
        ...
        LOGGER.debug("### FilehandlingServiceTest testCaching1 File is {}", obj.exists());
