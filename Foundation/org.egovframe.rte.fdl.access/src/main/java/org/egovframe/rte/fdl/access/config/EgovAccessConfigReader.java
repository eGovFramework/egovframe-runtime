...
/**
 * egov-access properties 설정 파일을 읽는 유틸리티 클래스
 *
 * <p>Desc.: properties 설정 파일을 읽어 EgovAccessConfig 객체로 변환하는 클래스</p>
 *
 * @author 유지보수
 * @version 6.0
 * @since 2025.06.01
 */
public class EgovAccessConfigReader {

    private static final Logger LOGGER = LoggerFactory.getLogger(EgovAccessConfigReader.class);
    private static final String DEFAULT_CONFIG_PATH = "egovframework/egovProps/conf/egov-access-config.properties";
     ...
     */
    public EgovAccessConfigReader(String configPath, ResourceLoader resourceLoader) {
        this.configPath = (configPath != null && !configPath.trim().isEmpty()) ? configPath.trim() : null;
        this.resourceLoader = resourceLoader;
    }

    /**
     * 지정된 경로의 설정 파일을 읽어 EgovAccessConfig 객체를 반환.
     ...
     */
    public EgovAccessConfig readConfig() {
        ...
        LOGGER.debug("Using default access configuration (no file found for path: {})", path);
     ...
     * 기본 설정 객체를 생성
     */
    private EgovAccessConfig createDefaultConfig() {
        EgovAccessConfig config = new EgovAccessConfig();
        config.setId("egovAccessConfig");
        config.setGlobalAuthen("session");
        config.setMappingPath("/**/*.do");
        config.setDataSource("dataSource");
        config.setLoginUrl("/uat/uia/egovLoginUsr.do");
        config.setAccessDeniedUrl("/uat/uia/egovLoginUsr.do?auth_error=1");
        config.setRequestMatcherType("regex");
        config.setExcludeList("/uat/uia/**, /index.do");
        config.setSqlAuthorityUser("SELECT USER_ID AS userid, AUTHOR_CODE AS authority FROM AUTHORITIES");
        config.setSqlRoleAndUrl("SELECT r.ROLE_PTTRN AS url, a.AUTHOR_CODE AS authority FROM ROLES r INNER JOIN AUTHROLES a ON r.ROLE_CODE = a.ROLE_CODE ORDER BY r.ROLE_SORT");
        LOGGER.debug("Use Basic Access Configuration");
        return config;
    }
