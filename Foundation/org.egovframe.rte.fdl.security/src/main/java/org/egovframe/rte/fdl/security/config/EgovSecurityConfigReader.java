...
/**
 * egov-security properties 설정 파일을 읽는 유틸리티 클래스
 *
 * <p>Desc.: properties 설정 파일을 읽어 EgovSecurityConfig 객체로 변환하는 클래스</p>
 *
 * @author 유지보수
 * @version 6.0
 * @since 2025.06.01
 */
public class EgovSecurityConfigReader {

    private static final Logger LOGGER = LoggerFactory.getLogger(EgovSecurityConfigReader.class);
    private static final String DEFAULT_CONFIG_PATH = "egovframework/egovProps/conf/egov-security-config.properties";
    ...
    public EgovSecurityConfigReader() {
        this(null, null);
    }
     ...
     */
    public EgovSecurityConfigReader(String configPath, ResourceLoader resourceLoader) {
        this.configPath = (configPath != null && !configPath.trim().isEmpty()) ? configPath.trim() : null;
        this.resourceLoader = resourceLoader;
    }

    /**
     * 지정된 경로의 설정 파일을 읽어 EgovSecurityConfig 객체를 반환.
     ...
     */
    public EgovSecurityConfig readConfig() {
        ...
        LOGGER.debug("Using default security configuration (no file found for path: {})", path);
     ...
     * 기본 설정 객체를 생성
     */
    private EgovSecurityConfig createDefaultConfig() {
        EgovSecurityConfig config = new EgovSecurityConfig();
        config.setId("egovSecurityConfig");
        config.setLoginUrl("/cvpl/EgovCvplLogin.do");
        config.setLogoutSuccessUrl("/cvpl/EgovCvplLogin.do");
        config.setLoginFailureUrl("/cvpl/EgovCvplLogin.do?login_error=1");
        ...
        config.setDefaultTargetUrl("/cvpl/EgovCvplLogin.do");
        ...
        LOGGER.debug("Use Basic Security Configuration");
        return config;
    }
