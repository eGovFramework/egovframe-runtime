 ...
 * @since 2025.06.01
 */
public class UsersQueryFactoryBean implements FactoryBean<String> {

    private static final String DEFAULT_QUERY = "select user_id, password, enabled, users.* from users where user_id = ?";
    private final EgovSecurityConfig config;

    public UsersQueryFactoryBean(EgovSecurityConfig config) {
        this.config = config;
    }
    ...
    public String getObject() {
        if (ObjectUtils.isEmpty(config)) {
            throw new NoSuchBeanDefinitionException("### UsersQueryFactoryBean getJdbcUsersByUsernameQuery not found.");
        }
