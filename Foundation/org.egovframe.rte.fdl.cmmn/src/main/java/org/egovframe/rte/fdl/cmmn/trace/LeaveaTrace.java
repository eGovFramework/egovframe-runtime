...
public class LeaveaTrace {
     ...
     * @param LOGGER            로그객체지정
     */
    public void trace(Class<?> clazz, MessageSource messageSource, String messageKey, Object[] messageParameters, Locale locale, Logger LOGGER) {
        String message = messageSource.getMessage(messageKey, messageParameters, null, locale);

        if (LOGGER != null) {
            LOGGER.debug("### LeaveaTrace trace() this.message : {}", message);
        }
