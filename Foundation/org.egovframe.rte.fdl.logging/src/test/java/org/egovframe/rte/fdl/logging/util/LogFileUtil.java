...
public class LogFileUtil {
    ...
    public static String[] getTailLines(File logFile, int paramLines) throws Exception {
        int lines = paramLines;
        ...
        while (lines > 0) {
            for (int i = 1; i <= fileLength; i++) {
                ...
                if (file.read() == '\n') {
                    if (isDummy) {
                        isDummy = false;
                        // 거꾸로 scan 하며 dummy가 아닌 뉴라인을 만나면 그 위치에서 readLine 을 하면 원하는 해당 라인 데이터 임
                    } else {
                        if (lines > 0) {
                ...
                // 뉴라인을 만날때 까지 뒤에서 부터 한 바이트씩 늘려가며 포인터 이동. cf.) seek 는 파일시작지점부터의 offset 임에 유의
