		...
		try {
			...
			try {
				...
				if (rs != null) try { rs.close(); }
					catch(SQLException ex) {LOGGER.error("[SQLException] ResultSet Next Runing : "+ ex.getMessage());}
					catch(Exception ex) {LOGGER.error("["+ex.getClass()+"] ResultSet Next Runing : " + ex.getMessage());}
				//2017.02.14 장동한 시큐어코딩(ES)-오류 상황 대응 부재[CWE-390, CWE-397]s
