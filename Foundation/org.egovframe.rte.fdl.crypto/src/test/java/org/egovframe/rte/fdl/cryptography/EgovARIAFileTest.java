...
public class EgovARIAFileTest {
    ...
    public void EgovARIAFilVerify() throws Exception{
		...
		String sPath = this.getClass().getResource("/META-INF/spring/file").getFile();
		System.out.println("##### file Path >>> " + sPath);
    	cryptoService.encrypt(new File(sPath+"/test.txt"), sAlgorithmKey, new File(sPath+"/test2.txt"));
    	cryptoService.decrypt(new File(sPath+"/test2.txt"), sAlgorithmKey, new File(sPath+"/test3.txt"));
